...
class MongoDB:
    ...
    def update_message(self, message, msg_id, mentioned_id):
        try:
            result = self.mentions.update_one({"msgId": msg_id, "mentioned_id": mentioned_id}, {"$set": {"message_text": message}})
    ...
    def delete_mention(self, msg_id, mentioned_id):
        try:
            return self.mentions.delete_one({"msgId": msg_id, "mentioned_id": mentioned_id})
        except Exception as e:
            logging.error(f"Error while deleting mention: {e}")
            raise
